...
TInfoAtomo lex_next(void)
{
    ...
    if (c == EOF)
    {
        token = criar_token(FIM_ARQUIVO, "EOF");
        if (!eof_ja_registrado)
        {
            log_token_write(&token);
            eof_ja_registrado = 1;
        }
        return token;
    }
    ...
    else
    {
        ...
        case '<':
        case '.':
            token = ler_operador(c);
            break;
        default:
            token = criar_token(ERRO, NULL);
    ...
    if (token.atomo == ERRO)
    {
        diag_error_lexico(token.linha, "token invalido", token.texto);
    }
    return token;
}
